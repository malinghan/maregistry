...
#### 1. 启动单节点实例
```bash
# 添加集群配置到application.properties
echo "maregistry.server-list[0]=http://localhost:8081" >> src/main/resources/application.properties
...
```properties
maregistry.server-list[0]=http://localhost:8081
maregistry.server-list[1]=http://localhost:8082
maregistry.server-list[2]=http://localhost:8083
```
